...
#best(subset(credit.train.balanced, select = -c(PROFIT_LOSS,DEFAULT)), maximize = FALSE)
caret::sbf(subset(credit.train.balanced, select = -c(PROFIT_LOSS,DEFAULT)), credit.train.balanced$DEFAULT)
#results <- rfe(subset(credit.train.balanced, select = -c(PROFIT_LOSS,DEFAULT)), credit.train.balanced$DEFAULT, sizes=c(1:8), rfeControl=control)
...
drop.cor = c('NB_INTR_1M','REV_BT','MNT_DEMANDE','NUM_EMPTY_STRINGS','NB_ER_12MS','NB_COUR')
reg <- glm(DEFAULT ~ ., data = subset(credit.train.balanced, select = -c(PROFIT_LOSS)),family="binomial")
step.reg <- step(reg)
